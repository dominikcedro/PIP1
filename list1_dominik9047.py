...
disclaimer: I tried to implement Google PEP 8 for docstrings setup.
'''
def weekday(day, month, year):
    ...
    Parameters:
    day : integer, day of the week: 0 for Sunday, 1 for Monday,
         2 for Tuesday, and so forth..
    month : integer, describes a month, use 1 for January,
         2 for February, and so o.
    year : integer , year as an integer.

    Returns:
    Day of the week : string, that's the day the user was searching for.
...
def random_walk(n):
    ...
    while sqrt(position[0]**2 + position[1]**2) < sqrt(n**2):
        move = randrange(4)
        if move > 1:
            if move == 2:
